...
import { normalizeFromAddress } from '../services/smtpConfig.js';
import { invalidateReadNextCache } from '../services/readNext.js';
import { rebuildBookCatalog } from '../services/bookCatalogSync.js';
import { stampPageContentFreshness } from '../services/contentFreshness.js';
...
router.put('/pages/:slug', requireAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  if (slug === 'email-settings' && contentObj && typeof contentObj === 'object') {
    const fromRaw = (contentObj as Record<string, unknown>).fromEmail;
    const smtpUser = (contentObj as Record<string, unknown>).smtpUser;
    if (typeof fromRaw === 'string' && fromRaw.trim()) {
      try {
        (contentObj as Record<string, unknown>).fromEmail = normalizeFromAddress(
          fromRaw,
          typeof smtpUser === 'string' ? smtpUser : undefined
        );
      } catch {
  ...
  try {
    if (slug === 'blog' || slug === 'recommendations' || slug === 'musings') {
      const existing = await Page.findOne({ slug }, { content: 1 });
      const existingContent =
        existing?.content && typeof existing.content === 'object' && !Array.isArray(existing.content)
          ? (existing.content as Record<string, unknown>)
          : null;
      contentObj = stampPageContentFreshness(slug, contentObj, existingContent);
    }
